...
const serializers: bunyan.Serializers = {
  req: (req: IncomingMessage) => {
    ...
    const headers: Record<string, string> = cloneDeep(req.headers) as Record<string, string>;
...
function requestLogMiddleware(): Middleware {
  return async (context: KoaContext, next: () => Promise<any>) => {
    ...
    context.log.info({
      ...
      req: { method: context.req.method, url: context.req.url, route: (context as any)['_matchedRoute'] },
    });
  };
}
