import React, { forwardRef, useId } from 'react';
import clsx from 'clsx';

import { ChevronRightIcon, XCloseIcon } from '../icons';

import styles from './input.module.scss';

export interface InputProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'> {
  value: string;
  label?: string;
  size?: 'sm';
  startAdornment?: React.ReactNode;
  endAdornment?: React.ReactNode;
  startIcon?: React.ReactNode;
  endIcon?: React.ReactNode;
  fullWidth?: boolean;
  helperText?: string;
  errorText?: string;
  className?: string;
  isClearable?: boolean;
  onClear?: () => void;
}

export const Input = forwardRef<HTMLInputElement, InputProps>((props, ref) => {
  const {
    name,
    value,
    label,
    size = 'sm',
    startAdornment: StartAdornment,
    endAdornment: EndAdornment,
    startIcon: StartIcon,
    endIcon: EndIcon,
    errorText,
    helperText,
    autoComplete = 'off',
    type = 'text',
    className,
    onClick,
    isClearable,
    onClear,
    ...inputProps
  } = props;
  const id = useId();

  const isError = Boolean(errorText);

  const handleClear = (event: React.MouseEvent<HTMLDivElement>) => {
    event.stopPropagation();
    inputProps?.onChange?.({ target: { name, value: '' } } as React.ChangeEvent<HTMLInputElement>);
    onClear?.();
  };

  const message = isError ? errorText : helperText;
  return (
    <div className={clsx(styles.root, className, isError && styles.root_error)}>
      {label && (
        <label className={styles.root__label} htmlFor={id}>
          {label}
        </label>
      )}
      <div className={clsx(styles.root__input, styles.input, isError && styles.input_error)} onClick={onClick}>
        {StartAdornment}
        <div
          className={clsx(
            styles.input__content,
            Boolean(StartAdornment) && styles.input__content_withStartAdornment,
            Boolean(EndAdornment) && styles.input__content_withEndAdornment,
          )}
        >
          {StartIcon && <div className={styles.input__icon}>{StartIcon}</div>}
          <input
            {...inputProps}
            type={type}
            id={id}
            name={name}
            value={value}
            className={clsx(styles.input__field)}
            autoComplete={autoComplete}
            ref={ref}
            aria-invalid={isError || undefined}
          />
          {EndIcon && <div className={styles.input__icon}>{EndIcon}</div>}
        </div>
        {isClearable && value.length > 0 && <Clear onClick={handleClear} />}
        {EndAdornment}
      </div>
      {message && <p className={styles.root__helperText}>{message}</p>}
    </div>
  );
});

Input.displayName = 'Input';

export interface DropdownProps {
  text?: string;
  onClick: () => void;
  className?: string;
}

export function Dropdown({ text, onClick, className }: DropdownProps): JSX.Element {
  return (
    <div className={clsx(styles.dropdown, className)} onClick={onClick}>
      {text && <span dangerouslySetInnerHTML={{ __html: text }} className={styles.dropdown__text} />}
      <ChevronRightIcon className={styles.dropdown__arrow} />
    </div>
  );
}

export interface TextEndAdornmentProps {
  text?: string;
  className?: string;
}

export function TextEndAdornment({ text, className }: TextEndAdornmentProps): JSX.Element {
  return (
    <div className={clsx(styles.dropdown, className)}>
      {text && <span dangerouslySetInnerHTML={{ __html: text }} className={styles.dropdown__text} />}
    </div>
  );
}

export interface ClearProps {
  onClick: (event: React.MouseEvent<HTMLDivElement>) => void;
  className?: string;
}

export function Clear({ onClick, className }: ClearProps): JSX.Element {
  return (
    <div className={clsx(styles.clear, className)} onClick={onClick}>
      <XCloseIcon className={styles.clear__cross} />
    </div>
  );
}
