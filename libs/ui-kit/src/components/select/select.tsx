...
export function Select<IsMulti extends boolean>(props: SelectProps<IsMulti>) {
  ...
  const filterOption = useCallback(
    (option: FilterOptionOption<ISelectOption>, input: string): boolean => {
      return Boolean(
        (isShowAllOptions || (!isShowAllOptions && input.trim().length >= minimumInputLength)) &&
...
function DropdownIndicator(): JSX.Element {
  return (
    <div className={styles.dropdownIndicator}>
      <ChevronRightIcon className={styles.dropdownIndicator__arrow} />
    </div>
  );
}
function ClearIndicator(props: ClearIndicatorProps): React.ReactElement {
  const {
  ...
  return (
    <div {...restInnerProps} ref={ref} className={styles.clearIndicator}>
      <XCloseIcon className={styles.clearIndicator__cross} />
    </div>
  );
}
